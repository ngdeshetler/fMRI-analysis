...
for g=1:length(SUBS)
    ...
    num_runs=8;
    mask_file_pre_a=sprintf('/space/raid6/data/rissman/Natalie/MATCH_MISMATCH/%s/masks/runRUN/RAD/SIDE_hipp_ashs.nii.gz',sub_ID);
    mask_file_pre_b=sprintf('/space/raid6/data/rissman/Natalie/MATCH_MISMATCH/%s/masks/runRUN/RAD/ASHS_SIDE_PHG.nii.gz',sub_ID);
    
    for c=1:length(SIDEs)
        
        mask_file_a=strrep(mask_file_pre_a,'SIDE',SIDEs{c});
        mask_file_b=strrep(mask_file_pre_b,'SIDE',SIDEs_alt{c});
            ...
            if exist(file_name_b)~=0
                ...
                if exist(mask_file_name_a)~=0
                    ...
                    for m=1:datasize_b(4)
                        ...
                        HIPP_PHG_corr.(sub_ID).(sprintf('%s_hipp',SIDEs{c})).(sprintf('block%d',n)).(sprintf('trial%d',all_variables.(sub_ID).cue.block(n).trials(m)))=mean(holder_a);
                        HIPP_PHG_corr.(sub_ID).(sprintf('%s_PHG',SIDEs{c})).(sprintf('block%d',n)).(sprintf('trial%d',all_variables.(sub_ID).cue.block(n).trials(m)))=mean(holder_b);
...
for g=1:length(SUBS)
    ...
    num_runs=8;
    mask_file_a=sprintf('/space/raid6/data/rissman/Natalie/MATCH_MISMATCH/%s/masks/runRUN/RAD/hipp_ashs.nii.gz',sub_ID);
    mask_file_b=sprintf('/space/raid6/data/rissman/Natalie/MATCH_MISMATCH/%s/masks/runRUN/RAD/PHG_ashs.nii.gz',sub_ID);
        ...
        if exist(file_name_b)~=0
            ...
            if exist(mask_file_name_a)~=0
                ...
                for m=1:datasize_b(4)
                    ...
                    HIPP_PHG_corr.(sub_ID).hipp.(sprintf('block%d',n)).(sprintf('trial%d',all_variables.(sub_ID).cue.block(n).trials(m)))=mean(holder_a);
                    HIPP_PHG_corr.(sub_ID).PHG.(sprintf('block%d',n)).(sprintf('trial%d',all_variables.(sub_ID).cue.block(n).trials(m)))=mean(holder_b);
...
save('HIPP_PHG_corr_all','HIPP_PHG_corr')
end