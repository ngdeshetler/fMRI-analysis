...
for g=1:length(SUBS)
    ...
    mask_file_pre_b=sprintf('/space/raid6/data/rissman/Natalie/MATCH_MISMATCH/%s/masks/runRUN/RAD/ASHS_SIDE_ERC.nii.gz',sub_ID);
            ...
            if exist(file_name_b)~=0
                ...
                if exist(mask_file_name_a)~=0
                    ...
                    for m=1:datasize_b(4)
                        ...
                        HIPP_ERC_corr.(sub_ID).(sprintf('%s_hipp',SIDEs{c})).(sprintf('block%d',n)).(sprintf('trial%d',all_variables.(sub_ID).cue.block(n).trials(m)))=mean(holder_a);
                        HIPP_ERC_corr.(sub_ID).(sprintf('%s_ERC',SIDEs{c})).(sprintf('block%d',n)).(sprintf('trial%d',all_variables.(sub_ID).cue.block(n).trials(m)))=mean(holder_b);
...
for g=1:length(SUBS)
    ...
    mask_file_b=sprintf('/space/raid6/data/rissman/Natalie/MATCH_MISMATCH/%s/masks/runRUN/RAD/ERC_ashs.nii.gz',sub_ID);
        ...
        if exist(file_name_b)~=0
            ...
            if exist(mask_file_name_a)~=0
                ...
                for m=1:datasize_b(4)
                    ...
                    HIPP_ERC_corr.(sub_ID).hipp.(sprintf('block%d',n)).(sprintf('trial%d',all_variables.(sub_ID).cue.block(n).trials(m)))=mean(holder_a);
                    HIPP_ERC_corr.(sub_ID).ERC.(sprintf('block%d',n)).(sprintf('trial%d',all_variables.(sub_ID).cue.block(n).trials(m)))=mean(holder_b);
...
save('HIPP_ERC_corr_all','HIPP_ERC_corr')
end